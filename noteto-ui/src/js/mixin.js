...
export default {
  ...
  methods: {
    ...
    changeEvent(field) {
      console.log(field, this.root);
      if (field == "qty") {
        eval(
          "this.root['totalAmount']=this.root['itemList'].reduce((sum,row) => {return sum + row.qty * row.unitPrice},0)"
        );
      }
      if (field == "unitPrice") {
        eval(
      ...
      backendService
        ...
        .catch((err) => {
          ...
          setTimeout(() => {
            this.setTimeoutLoading = false;
            if (code == 404) {
              eventBus.$emit("setSnackbar", "Entry not found!", "error");
            } else if (code == 401) {
              eventBus.$emit("setSnackbar", "No access to the entry!", "error");
            }
          }, 1000);
        });
    },
    ...
    getEntryText(field, value) {
      ...
      if (field.type == "multipleSelect") {
        ...
        return (
          field.options.prefix + " " + value.toFixed(field.options.precision)
        );
      } else if (field.type.includes("weight")) {
        return (
          value.toFixed(field.options.precision) + " " + field.options.suffix
        );
    ...
    runSetAutomations() {
      ...
      for (let i = 0; i < automations.length; i++) {
        if (automations[i].type == "set") {
          this.setValue(automations[i].actField, automations[i].actValue);
        }
      }
    },
