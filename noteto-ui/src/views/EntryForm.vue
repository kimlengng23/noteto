      ...
      <v-card-title class="d-flex justify-space-between">
        ...
        <div class="mt-3">
          <v-btn
            ...
            depressed
            @click="clearEntry"
            ><i class="fas fa-eraser mr-2"></i>Clear</v-btn
          >
          <v-btn
            v-show="isNew"
            rounded
        ...
        ><v-row v-for="(row, idxI) in rows" :key="`row-${idxI}`">
          ...
          <v-col v-for="(col, idxJ) in row.cols" :key="`col-${idxJ}`">
            ...
            <div v-else-if="fieldToField[col.field].type === 'list'">
              ...
              >
                <v-col
                  v-for="(field, lIdxJ) in fieldToField[col.field].listFields"
                  :key="`lCol-${lIdxJ}`"
                >
                  <v-text-field
                  ...
                  <vuetify-money
                    ...
                    prefix="$"
                    :label="field.displayName"
                    v-model.number="entry[col.field][lIdxI][field.value]"
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field
                        :value="
                          formatDate(entry[col.field][lIdxI][field.value])
                        "
                        :label="field.displayName"
                        persistent-hint
                        prepend-icon="mdi-calendar"
                    ...
                    <v-date-picker
                      v-model="entry[col.field][lIdxI][field.value]"
                      no-title
                      @input="datePicker[`${field.value}-${lIdxI}`] = false"
                    ></v-date-picker>
                  </v-menu>
                </v-col>
                <v-col>
