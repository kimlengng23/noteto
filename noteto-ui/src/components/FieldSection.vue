      ...
      <v-card-text>
        ...
        <div class="d-flex justify-end">
          <v-btn
            ...
            ><i class="fas fa-heading mr-2"></i>Update Header</v-btn
          >
        </div>
        ...
        <v-text-field label="Display Name" v-model="displayName" outlined>
        </v-text-field>
        <v-autocomplete
          label="Field Type"
          :items="fieldTypes"
...
export default {
  ...
  methods: {
    ...
    getAttributes(databaseValue) {
      ...
      backendService.getUsersByDatabase(databaseValue).then((response) => {
      ...
      backendService.updateField(field).then(() => {
        if (
          ...
          backendService.updateChoices(this.choices).then((response) => {
            this.$store.commit("setChoicesInDatabaseToChoices", response.data);
          });
        }
        eventBus.$emit(
          "setSnackbar",
